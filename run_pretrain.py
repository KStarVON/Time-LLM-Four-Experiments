...
with open('./config/config_ZERO-SHOT_ETTh1_ETTh2_pred96.yaml', 'r') as file:
...
accelerator = Accelerator(cpu=False, mixed_precision="no", deepspeed_plugin=None)

def test_pred_result(args, accelerator, model, vali_data, vali_loader, setting):
    preds = []
    trues = []
    print('loading model')
    model.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting + '-'+ args.model_comment, 'checkpoint.pth')))
    model.eval()
    with torch.no_grad():
...
for ii in range(args.itr):
    ...
    test_pred_result(args, accelerator, model, test_data, test_loader, setting)
